...
public class emp_ExitLot {
    public static void exitlot(BufferedReader reader,Connection conn, String emp_id) throws NumberFormatException, IOException, SQLException
    {
        ...
        /* Check if the employee has a valid permit*/
        String sql = "Select EXPIRETIME,PERMITNO from NONVISITORPERMIT where UNIVID = \'" + emp_id + "\'";
        rs = st.executeQuery(sql); 

        if(!rs.next()){
            System.out.println("Invalid Request. No permit foudn for the specified Employee.\nSwitching back to MAIN MENU");
            return ;
        }
        ExpireTime = rs.getTimestamp("EXPIRETIME");
        
        PermitNo = rs.getString("PERMITNO");
        //System.out.println(ExpireTime.toString() + " " + PermitNo + " " + emp_id);
        ...
        sql = "Select * from ASSIGNMULTIPLE WHERE PARKEDAT IS NOT NULL AND SPACENO IS NOT NULL AND LOTNAME IS NOT NULL AND UNIVID = \'" + emp_id + "\' AND VEHICLENO = \'" + VehicleNo +"\'";
        rs = st.executeQuery(sql); 
        ...
        if (res > 0){
            /*Indcates the epire time has crossed, while exiting the lot. Issue Expired permit citation*/
            ...
            IssueCitation.issuecitation(conn, emp_id, VehicleNo, LotName, model, car_color, "Expired Permit", 25, "Employee"); 
        }
        ...
        try{
            ...
            try {
                sql = "UPDATE ASSIGNMULTIPLE SET LOTNAME = NULL, SPACENO = NULL,PARKEDAT = NULL WHERE SPACENO = ? AND VEHICLENO=? AND LOTNAME = ?";
                ps = conn.prepareStatement(sql);
                ps.setInt(1, SpaceNo);
                ...
                if (rs != null) {
                    System.out.println("Parking Space unallocated for the employee");
                    trans2 = true;
                } else {
                    System.out.println("Unable to de-allocated space for the employee");
                    trans2 = false;
                }
